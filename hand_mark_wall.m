...
i=175;
num='two';
im_tif=imread(sprintf('./tif-extra/%04d.tif',i));
im=imread(sprintf('./png-extra/%04d.png',i));
...
[ind_row_dist,ind_col_dist]=find(dist_img<120);
mask_dist=zeros(hei,wid);
mask_dist(sub2ind(size(mask_dist),ind_row_dist,ind_col_dist))=1;


mask_wall=mask_dist;
save(['./seg_wall_extra_' num sprintf('/img_%04d.mat',i)],...
        'mask_wall');
mask_vis=cat(3,mask_wall*225,mask_wall*225,mask_wall*0);
