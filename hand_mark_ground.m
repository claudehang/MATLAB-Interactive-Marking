...
im_tif=imread(sprintf('./tif-person/%04d.tif',i));
im=imread(sprintf('./png-person/%04d.png',i));
...
[ind_row_dist,ind_col_dist]=find(dist_img<60);
mask_dist=zeros(hei,wid);
mask_dist(sub2ind(size(mask_dist),ind_row_dist,ind_col_dist))=1;


mask_floor=mask_dist;
save(['./seg_floor_person' sprintf('/img_%04d.mat',i)],...
        'mask_floor');
mask_vis=cat(3,mask_floor*225,mask_floor*225,mask_floor*0);
